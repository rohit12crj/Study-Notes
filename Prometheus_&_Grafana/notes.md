...
difference between node exporter pod (for hardware metrics) , kube state metrics pods ( from API server ) , custom metrics (from /metrics endpoint --> application level metrics ) in prometheus ? There are also db exporters which are not installed by default ?
