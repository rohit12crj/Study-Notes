...
difference between node exporter pod (for hardware metrics) , kube state metrics pods , appliation metrics (/metrics endpoint ) in prometheus ? There are also db exporters which are not installed by default ?
